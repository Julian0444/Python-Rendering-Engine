...
def main():
    ...
    vertex_shader = os.path.join(shader_dir, 'basic.vert')
    fragment_shader = os.path.join(shader_dir, 'basic.frag')
    sprite_vertex_shader = os.path.join(shader_dir, 'sprite.vert')
    sprite_fragment_shader = os.path.join(shader_dir, 'sprite.frag')
    
    shader = ShaderProgram(window.ctx, vertex_shader, fragment_shader)
    shader_sprite = ShaderProgram(window.ctx, sprite_vertex_shader, sprite_fragment_shader)
    ...
    cube1 = Cube((2.5, 0, 5), (0, 0, 0), (1, 1, 1), name="Cube1")
    ...
    camera = Camera((0, 0, 12), (0, 0, 0), (0, 1, 0), 45, WIDTH / HEIGHT, 0.01, 100.0)
